...
class EuroScrapas:
    ...
    def implement_scrapping_process(self):
        ...
        for index, sc in enumerate(self.season_codes):
            ...
            while not season_end:
                ...
                # save meta_data
                with open(fr"../euroleague_json_data/{sc}/{sc}_meta_data_{self.datetime_now}.json", "w") as output_file:
    ...
    def initialize_process(self, index, sc):
        ...
        # create directories
        if not os.path.exists(fr"../euroleague_json_data/{sc}"):
            os.makedirs(fr"../euroleague_json_data/{sc}/success")
            os.makedirs(fr"../euroleague_json_data/{sc}/failure")
    ...
    def request_extract_load_update(self, gc, sc, process_start_time, meta_data_dict):
        ...
        for api in self.euroleague_apis:
            ...
            try:
                ...
                # load data
                with open(fr"../euroleague_json_data/{sc}/success/{json_filename}.json", "w") as output_file:
                ...
                with open(fr"../euroleague_json_data/{sc}/failure/{sc}_failed_extractions_{self.datetime_now}.txt", 'a') as failure_file:
    ...
    def get_info_from_header_api(self, response_dict, meta_data_dict, url, gc, sc, process_start_time):
        ...
        if gc == 1:
            print("")
        print(f"SeasonCode: {sc}  ---  Phase: {Phase}  ---  Round: {Round}  ---  GameCode:", "{:03d}".format(gc), \
