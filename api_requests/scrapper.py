...
class EuroScrapas:
    ...
    def implement_scrapping_process(self):
        ...
        for index, sc in enumerate(self.season_codes):
            ...
            while not season_end:
                ...
                # save meta_data
                with open(fr"../data/{self.competition}_json/{sc}/{sc}_meta_data_{self.datetime_now}.json", "w") as output_file:
    ...
    def initialize_process(self, index, sc):
        ...
        # create directories
        if not os.path.exists(fr"../data/{self.competition}_json/{sc}"):
            os.makedirs(fr"../data/{self.competition}_json/{sc}/success")
            os.makedirs(fr"../data/{self.competition}_json/{sc}/failure")
    ...
    def request_extract_load_update(self, gc, sc, process_start_time, meta_data_dict):
        ...
        for api in self.euroleague_apis:
            ...
            try:
                ...
                # load data
                with open(fr"../data/{self.competition}_json/{sc}/success/{json_filename}.json", "w") as output_file:
                ...
                with open(fr"../data/{self.competition}_json/{sc}/failure/{sc}_failed_extractions_{self.datetime_now}.txt", 'a') as failure_file:
