...
class ConfigParser(ArgumentParser):
    ...
    def __init__(self, year_today, available_season_codes):
        ...
        super().__init__(prog='postgreSQL',
                         description='load the extracted euroleague/eurocup data (format of data: json files) to a postgreSQL database')
    ...
    def parse_arguments(self, args):
        ...
        try:
            ...
            if not self.season_codes:
                ...
                exit()
            elif not set(self.season_codes).intersection(self.available_season_codes[self.competition]):
                print("\nRequested data does not exist: the available season codes are", sorted(self.available_season_codes[self.competition]), "\n")
                exit()
            else:
                self.season_codes = sorted(list(set(self.season_codes).intersection(self.available_season_codes[self.competition])))
